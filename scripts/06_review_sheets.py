...
for row in (["item", "value", "source", "url"],
            ...
            ["Cruise type", "pre-marking stand exam: plots tally all trees; the sale-cruise standards are applied as if the whole tallied volume were designated for cutting", "demonstration convention; a sale cruise would tally designated trees only", "https://github.com/williamsteinley/plumas-lidar-harvest-planning/blob/main/docs/methods.md"],
            ...
            ["Data", "SIMULATED: every plot and tree is generated from LiDAR canopy metrics with six planted recording errors; cruiser code SIM, date field 'simulated', SIMULATED DATA watermark on every review-sheet page", "docs/methods.md", "https://github.com/williamsteinley/plumas-lidar-harvest-planning/blob/main/docs/methods.md"]):
    ws6.append(row)
