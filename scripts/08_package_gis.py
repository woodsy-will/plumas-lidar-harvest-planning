...
metadata table, and writes the yarding-class and planning-slope rasters alongside it as compressed, tiled
GeoTIFFs with overviews. docs/data_dictionary.md documents every layer and field.
...
DESC = {
    ...
    "rca_buffers": "Riparian conservation areas, 300 / 150 / 100 ft (Sierra Nevada Forest Plan Amendment), mapped only",
    ...
    "corridors": "Skyline corridors cast from each landing every 5 degrees (10 for tractor units) to a tailhold 100 ft past the far boundary; 3D lines follow the DTM",
...
for gp, layers in SRC:
    for lyr in layers:
        gdal.VectorTranslate(GPKG, os.path.join(WORK, gp), layers=[lyr], accessMode=None if first else "update", format="GPKG")
        first = False
...
for name in ("yarding_class", "slope_plan_pct", "canopy_cover_66ft", "dom_height_66ft"):
    ...
    gdal.Translate(dst, os.path.join(WORK, name + ".tif"), creationOptions=["COMPRESS=DEFLATE", "PREDICTOR=2", "TILED=YES", "BIGTIFF=IF_SAFER"])
